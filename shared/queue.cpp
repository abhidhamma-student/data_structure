...
public:
	...
	int Size() const
	{
		...
		if (front_ < rear_) // 일반적인 경우: rear-front
			return rear_ - front_;
		else if (front_ > rear_) // front가 rear 뒤에 있는 경우: 전체 - 비어있는 공간의 길이
	...
	void Resize() // 2배씩 증가
	{
		...
		int count = 1; // 앞에 front 공간을 띄우고 시작
		for(
			int i = (front_ + 1) % capacity_; // front+1 부터 시작해서 나머지 연산으로 원형으로 작동하도록 만든다
			i != (rear_ + 1) % capacity_; // (rear + 1)이 아닐때 까지 복사
			i = (i + 1) % capacity_ // 
		)
		{
